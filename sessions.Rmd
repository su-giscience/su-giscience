...
<!-- You can register to session 18 about R Markdown [there](https://www.eventbrite.fr/e/billets-grenoble-rug-working-session-n18-r-markdown-59569890145). -->
...
program <- tribble(
  ...
  "16/05/2019", "19/20", "Data manipulation with package {dplyr}", "Data manipulation is an essential step in any data analysis. Core package of the tidyverse environment, **{dplyr}** allows data manipulation in R using a consistent and easy to use set of verbs. In this tutorial we will learn how to pick observations by their values (`r filter()`), reorder the rows (`r arrange()`), pick variables by their name (`r select()`), create new variables with functions of existing variables (`r mutate()`), collapse many values down to a single summary (`r summarise()`) and combine all these operations using the `r %>%` (pipe) operator. This will help you solve the most common data manipulation challenges.", "M. Rolland",
